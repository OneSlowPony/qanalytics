...
from qanalytics.data import query_to_list
...
@analytics.route("/sites/<int:site_id>")
@login_required
def view_site_visits(site_id=None):
    ...
    return render_template("analytics/site.html", visits=data, site=site)
  

@analytics.route("/sites/<int:site_id>/visit", methods=("GET", "POST"))
...
@analytics.route("/sites", methods=("GET", "POST"))
@login_required
def view_sites():
    form = SiteForm()
    ...
    return render_template("analytics/sites.html", sites=results, form=form)
  
  
def _make_link(site_id):
  url = url_for(".view_site_visits", site_id=site_id)
  return Markup("<a href='{url}'>{name}</a>").format(url=url, name=site_id)
